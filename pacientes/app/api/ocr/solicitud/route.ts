...
import { extraerSolicitud } from "@/lib/anthropic/extract";
...
export async function POST(request: NextRequest) {
  ...
  try {
    storagePath = await subirDocumento("solicitudes", buffer, archivo.type, extension);
  } catch (e) {
  ...
  try {
    const extracted = await extraerSolicitud(
      buffer.toString("base64"),
      archivo.type as "image/jpeg" | "image/png" | "image/webp" | "image/gif"
    );
    return NextResponse.json({ extracted, storage_path: storagePath });
  } catch (e) {
    return NextResponse.json(
      {
        error:
