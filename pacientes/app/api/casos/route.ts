...
const CasoManualSchema = z.object({
...
export async function GET(request: NextRequest) {
  ...
  if (q) {
    const rutLimpio = normalizarRut(q);
    query = query.or(
      `paciente.rut.ilike.%${rutLimpio}%,paciente.nombre.ilike.%${q}%`
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  const parsed = CasoManualSchema.safeParse(body);
  if (!parsed.success) {
