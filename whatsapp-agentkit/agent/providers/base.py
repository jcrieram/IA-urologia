...
class ProveedorWhatsApp(ABC):
    ...
    async def validar_firma(self, request: Request, cuerpo: dict) -> bool:
        """
        Verifica que el webhook venga realmente del proveedor.

        El endpoint /webhook es público: sin esta comprobación cualquiera puede
        enviarle mensajes falsos, consumir tu cuota de Claude y suplantar a un
        paciente. Por defecto se permite (los proveedores sin firma la sobrescriben).
        """
        return True
